<x-app-layout>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr class="border-b">
                        ...
                        <th class="px-6 py-3 text-center" width="120">View Items</th>
                ...
                <tbody class="bg-white">
                    @if ($orders->isNotEmpty())
                        @foreach ($orders as $order)
                            <tr class="border-b">
                                <td class="px-6 py-3 text-left">{{ ($orders->total() - (($orders->currentPage() - 1) * $orders->perPage()) - $loop->index) }}</td>
                                <td class="px-6 py-3 text-left">{{ $order->customer->name }}</td>
                                <td class="px-6 py-3 text-left">{{ \Carbon\Carbon::parse($order->order_date)->format('d M, Y') }}</td>
                                <td class="px-6 py-3 text-left">{{ \Carbon\Carbon::parse($order->delivery_date)->format('d M, Y') }}</td>
                                ...
                                <td class="px-6 py-3 text-left">{{ \Carbon\Carbon::parse($order->created_at)->format('d M, Y') }}</td>
                                ...
                                <td class="px-6 py-3 text-center">
                                    @can('edit orders')
                                        <a href="{{ route('orders.edit', $order->id) }}" class="bg-gray-800 hover:bg-gray-700 text-sm rounded-md text-white px-3 py-2">Edit</a>
                                    @endcan
                                    @can('delete orders')
                                        <a href="javascript:void(0)" onclick="deleteOrder({{ $order->id }})" class="bg-red-700 text-sm rounded-md text-white px-3 py-2 hover:bg-red-600">Delete</a>
                                    @endcan
                                </td>
                                @endcanany
                            </tr>
    ...
    <div id="orderItemsModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-100 overflow-hidden">
                <!-- Modal Header -->
