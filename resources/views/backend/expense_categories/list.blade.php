                    ...
                    @if ($expenseCategories->isNotEmpty())
                        @foreach ($expenseCategories as $category)
                            <tr class="border-b">
                                ...
                                <td class="px-6 py-3 text-left">{{ \Carbon\Carbon::parse($category->created_at)->format('d M, Y') }}</td>
                                @canany(['edit expense categories', 'delete expense categories'])
                                <td class="px-6 py-3 text-center">
                                    @can('edit expense categories')
                                        <a href="{{ route('expense_categories.edit', $category->id) }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-600">Edit</a>
                                    @endcan
                                    @can('delete expense categories')
                                        <a href="javascript:void()" onclick="deleteCategory({{ $category->id }})" class="bg-red-700 text-sm rounded-md text-white px-3 py-2 hover:bg-red-600">Delete</a>
                                    @endcan
                                </td>
                                @endcanany
                            </tr>
