<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('users.index') }}" class="bg-gray-800 hover:bg-gray-700 text-sm rounded-md px-3 py-2 text-white flex justify-center items-center gap-1">
                            ...
                            <div class="grid grid-cols-4 gap-4 mb-4">
                                @if ($roles->isNotEmpty())
                                    @foreach ($roles as $role)
                                        <div class="mt-3">
                                            <input  type="checkbox" {{ $hasRoles->contains($role->id) ? 'checked' : '' }} name="roles[]" value="{{ $role->name }}" id="role-{{ $role->id }}"
